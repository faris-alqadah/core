...
Computes the lower neighbors of a concept using the \char`\"{}Cover\char`\"{} algorithm in Berry et al. 


\begin{DoxyParams}{Parameters}
{\em c} & the n-\/cluster containg the concept as c(s,t) \\
\hline
{\em g} & the relation graph for which (s,t) should be a context \\
\hline
{\em s} & the \char`\"{}source id\char`\"{} of the domain for which the concept a(s,t) is the current concept, the lower neighbors will be sub-\/sets of domain s \\
\hline
{\em t} & the \char`\"{}target id\char`\"{} of the domain for which the concept a(s,t) is the current concept, the lower neighbors will be super-\/sets of domain t \\
\hline
\end{DoxyParams}
...
\begin{DoxyParams}{Parameters}
{\em lrnrConcept} & the learner concept that we are trying match is lrnrConcept(s,t) \\
\hline
{\em g} & the star-\/shaped relation graph \\
\hline
{\em s} & the domain id of the articulation node and concept lrnrConcept(s,t) \\
\hline
{\em t} & the domain id of the other domain that makes up the learner concept \\
\hline
\end{DoxyParams}
...
In addtion the primes of each maxmod is computed, while we keep track of what maxmods dominate each non-\/dominating maxmod


\begin{DoxyParams}{Parameters}
{\em ctx} & The full context for which the non-\/dominating maxmods will be computed \\
\hline
{\em c} & the n-\/cluster that contains the concept with which the maxmods were generated \\
\hline
{\em maxmods} & list of maxmods \\
\hline
{\em primes} & this vector should be empty, and the routine will store the primes of the maxmods in here \\
\hline
{\em domInfo} & this vector should be empty, the route will store the index to which maxmods dominate every maxmod here \\
\hline
\end{DoxyParams}
...
Computes the upper neighbors of a concept using the \char`\"{}Cover\char`\"{} algorithm in Berry et al. 
...
{\em g} & the relation graph for which (s,t) should be a context \\
\hline
{\em s} & the \char`\"{}source id\char`\"{} of the domain for which the concept a(s,t) is the current concept, the upper neighbors will be super-\/sets of domain s \\
\hline
{\em t} & the \char`\"{}target id\char`\"{} of the domain for which the concept a(s,t) is the current concept, the upper neighbors will be sub-\/sets of domain t \\
\hline
\end{DoxyParams}
...
\item 
headers/nclusters/berry.h\item 
source/nclusters/berry.cpp\end{DoxyCompactItemize}
